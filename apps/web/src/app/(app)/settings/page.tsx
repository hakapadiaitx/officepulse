...
export default function SettingsPage() {
  ...
  const [digestSendMsg,   setDigestSendMsg]   = useState<{ type: "ok" | "err"; text: string } | null>(null);
  ...
  async function sendDigestNow() {
    ...
    setDigestSendMsg(null);
    try {
      const res = await fetch("/api/settings/send-digest", { method: "POST" });
      const d = await res.json().catch(() => ({}));
      if (res.ok) {
        setDigestSendMsg({ type: "ok", text: `Digest sent to ${d.sentTo} ✓` });
      } else {
        setDigestSendMsg({ type: "err", text: d.error ?? "Failed to send digest." });
      }
    } catch {
      setDigestSendMsg({ type: "err", text: "Network error. Please try again." });
    } finally {
      ...
      <div className="card p-6 space-y-4">
        <div className="flex items-start justify-between gap-4">
          <div>
            ...
            <p className="text-sm text-gray-500">
              Daily attendance summary emailed at 6 PM UTC. Sent to <strong>{user?.email}</strong>.
            </p>
          </div>
          <button
            role="switch"
        ...
        {!digestEnabled && !digestError && (
          <p className="text-xs text-gray-500 bg-gray-50 border border-gray-200 rounded-lg px-3 py-2">
            Toggle on to start receiving daily digests automatically at 6 PM UTC.
          </p>
        )}

        {digestEnabled && !digestError && (
          <p className="text-xs text-green-700 bg-green-50 border border-green-100 rounded-lg px-3 py-2">
            ✓ Digest is active — you&apos;ll receive a summary every day at 6 PM UTC.
          </p>
        )}
        ...
        <div className="flex items-center gap-3 pt-1 border-t border-gray-100">
          <button
            ...
            className="flex items-center gap-2 text-sm font-medium text-gray-700 border border-gray-200 hover:border-gray-300 px-3 py-1.5 rounded-lg transition-colors disabled:opacity-50"
          >
            {digestSending
          ...
          <p className="text-xs text-gray-400">Test the digest — sends today&apos;s report immediately.</p>
        </div>

        {digestSendMsg && (
          <p className={`text-xs rounded-lg px-3 py-2 border ${
            digestSendMsg.type === "ok"
